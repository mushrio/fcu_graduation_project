...
本專題開發一款用於工廠中辨識手寫文件表格內文字的行動應用程式，可框選表格範圍、切割欄位，並辨識表格內的手寫文字。系統支援離線運作、辨識結果輸出、本地歷史紀錄，適合用於資料數位化與表單管理自動化。
...
👉 [HandwritingRecognition-Debug.apk](https://drive.google.com/drive/folders/1V1p3cQWdsQYkPXdxqhjmih7J9T2xPEip?usp=sharing)

> ⚠️ 注意：此版本為 Debug 模式，用於展示與學術用途，可能包含開發用 Log 與未優化功能。拍照模式請確保不要有太多光線干擾，背景選用深色乾淨的背景，例如用黑色的衣服。

## 心得與收穫

這次的畢業專題中我在 Android Studio 上做了更多的嘗試。包括引入第三方開源套件並實作，自訂有其他功能的元件，以及整合同學撰寫好的模型與程式碼。這些都是很新鮮的經驗。開發過程中也碰到了許多挑戰，例如製作表格時，Android Studio 內沒有現成的元件可以滿足我們想要表現的方式，而且自己寫出來的可能會有潛在效能問題。最終我找到合適的第三方套件並成功整合，學會了如何閱讀套件文件、理解原始碼與實作整合。此外，我也曾遭遇因套件重複引用導致的 duplicated class 問題。透過查閱版本紀錄與社群留言，發現是套件內含重複的 class。這時我利用 `exclude` 指令排除衝突的模組，成功解決問題。最後我也想謝謝我的同學，因為我們之間主動討論以及提出並解決問題，讓整個專題進行得很順利。
...
負責部分：
* 邱何朋：開發辨識模型、撰寫 CallPython、YoloInference 及 python 程式碼。
* 羅勖恩：實作應用程式介面與邏輯、整體資料流處理、整合同學開發成果進專題中。
